...
This directory contains Gherkin feature files that define the expected behavior of the `yaml-reference-cli` tool. The tool processes YAML files with special tags (`!reference`, `!reference-all`, `!flatten`) and outputs resolved JSON documents.

## Overview

The `yaml-reference-cli` is a command-line tool that:
1. Takes a YAML file as input
2. Resolves all `!reference`, `!reference-all`, and `!flatten` tags
...
## 5. Protection Against Cyclical References
...
## 6. Basic Reference Access Restriction with "allowed" Paths
